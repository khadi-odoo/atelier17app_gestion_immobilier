...
@section('title', $picture ->exists ? 'Editer ' : 'Ajouter image à ce un bien ' )

@dump($picture)
@section('content')

<form class="vstack gap-2" action="{{ route( $picture ->exists ? 'admin.picture.update' : 'admin.picture.store', ['picture' =>  $property ] ) }}" method="post"  enctype="multipart/form-data"  >
    @csrf
    ...
    {{-- <input class ="btn btn-primary" type="submit" value=" @if($picture -> exists) Editer @else Créer @endif"> --}}
